...
setup_dict = {
    ...
    'clip_version_name': 'vision_transformer',
...
def main(config):
    ...
    # unique_scans = get_unique_scans(config.data_file)
    ...
    torch.save(text_features, f'{config.interim_save_path}/{config.rpn_mode}/normalized_text_features_{config.clip_version_name}_{config.data_mode}_{config.text_feature_mode}.pt')
    pp.pprint(text_features[0])

    unique_scans = get_unique_scans(config.data_file)
    pp.pprint(unique_scans[0])
    ...
    image_features = compute_image_features(unique_scans, scan_bbox_arr, config.floorplans, model, config.device, transform)
    torch.save(image_features, f'{config.interim_save_path}/{config.rpn_mode}/normalized_image_features_{config.clip_version_name}_{config.data_mode}.pt')
    ...
    image_features = torch.load(f'{config.interim_save_path}/{config.rpn_mode}/normalized_image_features_{config.clip_version_name}_{config.data_mode}.pt')
    text_features = torch.load(f'{config.interim_save_path}/{config.rpn_mode}/normalized_text_features_{config.clip_version_name}_{config.data_mode}_{config.text_feature_mode}.pt')

    similarity_scores_arr = get_similarity_scores(image_features, text_features, scan2idx)
    torch.save(similarity_scores_arr, f'{config.interim_save_path}/{config.rpn_mode}/similarity_scores_arr_{config.clip_version_name}_{config.data_mode}_{config.text_feature_mode}.pt')
    ...
    torch.save(best_bbox_arr, f'{config.processed_save_path}/{config.rpn_mode}/best_bbox_arr_{config.clip_version_name}_{config.data_mode}_{config.text_feature_mode}_all_floors.pt')
