...
def get_persepctive_matrices(nn_heading_angles):
    parameter_matrices = {} 
...
def map_xy_coordinate(x, y, nn_heading_angles, parameter_matrices):
    ...
    # This returns pano_slice_rank, x_coord, y_coord        
    return [target_image_number, best_x_index[-1], best_y_index[0]]
...
def main():
    ...
    parameter_matrices = get_persepctive_matrices(nn_heading_angles)
    bounding_boxes = torch.load(f'/data2/saaket/best_{mode}_bboxes_ViT_full_pano.pth')
    mapped_boxes = []
    for boxes_in_pano in tqdm(bounding_boxes):
        mapped_boxes_in_pano = []
        for box in boxes_in_pano:
            x1, y1, x2, y2 = box
            mapped_slice_1, mapped_x1, mapped_y1 = map_xy_coordinate(x1, y1, nn_heading_angles, parameter_matrices)
            mapped_slice_2, mapped_x2, mapped_y2 = map_xy_coordinate(x2, y2, nn_heading_angles, parameter_matrices)
            mapped_boxes_in_pano.append([mapped_slice_1, mapped_x1, mapped_y1, mapped_slice_2, mapped_x2, mapped_y2])
        mapped_boxes.append(mapped_boxes_in_pano)
    torch.save(mapped_boxes, f'/data2/saaket/mapped_best_{mode}_bboxes_ViT_full_pano.pth')
