...
def get_image_features(unique_pano_slices, bboxes, image_encoder, transform, device):
    slice_features = []
...
def main():
    ...
    bounding_boxes = np.load(f'/data2/saaket/best_{mode}_boxes_perspective.npy')
    ...
    image_features = get_image_features(unique_pano_slices[5000:], bounding_boxes[5000:], model, preprocess, device)
    torch.save(image_features, f'/data2/saaket/image_features_sdr_perspective_{mode}_end.pth')
